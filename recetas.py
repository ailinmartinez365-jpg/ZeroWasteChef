recetas = [

    # ============================================================
    # RECETAS ORIGINALES
    # ============================================================

    {
        "nombre": "Huevos con tomate",
        "imagen": "imagenes/huevos_tomate.png",
        "ingredientes": ["huevo", "tomate", "sal", "aceite"],
        "tiempo": 10,
        "nivel": "Principiante",
        "descripcion": "Una receta rápida y sencilla para aprovechar huevos y tomate.",
        "instrucciones": [
            "Lava y corta el tomate en cubos pequeños.",
            "Calienta un poco de aceite en una sartén.",
            "Agrega el tomate y cocina durante unos minutos.",
            "Bate los huevos con una pizca de sal.",
            "Agrega los huevos a la sartén y mezcla hasta que estén cocidos.",
            "Sirve caliente."
        ]
    },

    {
        "nombre": "Quesadillas de queso",
        "imagen": "imagenes/quesadillas_queso.png",
        "ingredientes": ["tortilla", "queso", "aceite"],
        "tiempo": 10,
        "nivel": "Principiante",
        "descripcion": "Quesadillas fáciles para aprovechar tortillas y queso.",
        "instrucciones": [
            "Coloca una tortilla sobre una superficie limpia.",
            "Agrega queso en una mitad de la tortilla.",
            "Dobla la tortilla por la mitad.",
            "Calienta un poco de aceite en una sartén.",
            "Cocina la quesadilla por ambos lados hasta que esté dorada y el queso se derrita.",
            "Sirve caliente."
        ]
    },

    {
        "nombre": "Pasta con tomate",
        "imagen": "imagenes/pasta_tomate.png",
        "ingredientes": ["pasta", "tomate", "sal", "aceite"],
        "tiempo": 20,
        "nivel": "Principiante",
        "descripcion": "Una pasta sencilla preparada con tomate.",
        "instrucciones": [
            "Cuece la pasta en agua con un poco de sal.",
            "Lava y corta el tomate.",
            "Calienta aceite en una sartén.",
            "Agrega el tomate y cocina durante unos minutos.",
            "Escurre la pasta y agrégala a la sartén.",
            "Mezcla bien y sirve caliente."
        ]
    },

    {
        "nombre": "Pasta cremosa",
        "imagen": "imagenes/pasta_cremosa.png",
        "ingredientes": ["pasta", "queso", "leche", "sal"],
        "tiempo": 20,
        "nivel": "Principiante",
        "descripcion": "Una pasta cremosa y fácil de preparar.",
        "instrucciones": [
            "Cuece la pasta en agua con un poco de sal.",
            "Calienta la leche en una sartén a fuego bajo.",
            "Agrega el queso y mezcla hasta que se derrita.",
            "Escurre la pasta.",
            "Agrega la pasta a la salsa de queso.",
            "Mezcla bien y sirve caliente."
        ]
    },

    {
        "nombre": "Arroz con verduras",
        "imagen": "imagenes/arroz_verduras.png",
        "ingredientes": ["arroz", "verduras", "sal", "aceite"],
        "tiempo": 25,
        "nivel": "Principiante",
        "descripcion": "Una opción práctica para aprovechar arroz y verduras.",
        "instrucciones": [
            "Lava y corta las verduras en trozos pequeños.",
            "Calienta un poco de aceite en una sartén.",
            "Agrega las verduras y cocina durante unos minutos.",
            "Incorpora el arroz previamente cocido.",
            "Agrega una pizca de sal.",
            "Mezcla y cocina hasta que todo esté bien caliente."
        ]
    },


    # ============================================================
    # NIVEL 1
    # ============================================================

    {
        "nombre": "Puré de papa cremoso con tocino",
        "imagen": "imagenes/pure_papa_tocino.png",
        "ingredientes": [
            "papa", "tocino", "mantequilla", "crema",
            "queso mozzarella", "ajo", "nuez moscada",
            "sal", "pimienta"
        ],
        "tiempo": 35,
        "nivel": "Principiante",
        "descripcion": "Puré cremoso de papa con tocino y queso para aprovechar ingredientes sencillos.",
        "instrucciones": [
            "Cuece las papas hasta que estén suaves.",
            "Fríe el tocino hasta que esté dorado y pícalo.",
            "Machaca las papas y agrega mantequilla y crema.",
            "Incorpora el ajo, la nuez moscada, sal y pimienta.",
            "Agrega el queso mozzarella y mezcla hasta que se derrita.",
            "Incorpora el tocino y sirve caliente."
        ]
    },

    {
        "nombre": "Tiras de sandwich dorado a la francesa",
        "imagen": "imagenes/tiras_sandwich_francesa.png",
        "ingredientes": [
            "pan", "mayonesa", "queso", "jamón",
            "huevo", "leche", "mantequilla", "sal"
        ],
        "tiempo": 20,
        "nivel": "Principiante",
        "descripcion": "Tiras de sandwich rellenas y doradas para aprovechar pan, queso y jamón.",
        "instrucciones": [
            "Aplana ligeramente las rebanadas de pan.",
            "Unta mayonesa y coloca queso y jamón.",
            "Enrolla o dobla el pan formando pequeños rollos.",
            "Bate el huevo con la leche y una pizca de sal.",
            "Pasa los rollos por la mezcla de huevo.",
            "Derrite mantequilla en una sartén y dora los rollos por todos sus lados."
        ]
    },

    {
        "nombre": "Bocadillos de pollo",
        "imagen": "imagenes/bocadillos_pollo.png",
        "ingredientes": [
            "pollo", "tocino", "aceite", "paprika",
            "sal", "pimienta", "ajo"
        ],
        "tiempo": 20,
        "nivel": "Principiante",
        "descripcion": "Pequeños bocadillos de pollo sazonados y envueltos con tocino.",
        "instrucciones": [
            "Corta el pollo en trozos pequeños.",
            "Sazona con paprika, ajo, sal y pimienta.",
            "Envuelve los trozos de pollo con tocino.",
            "Calienta un poco de aceite en una sartén.",
            "Cocina los bocadillos hasta que el pollo esté completamente cocido.",
            "Sirve calientes."
        ]
    },

    {
        "nombre": "Mini rollitos de pizza",
        "imagen": "imagenes/mini_rollitos_pizza.png",
        "ingredientes": [
            "tortilla", "salsa de tomate", "queso",
            "salami", "mantequilla"
        ],
        "tiempo": 20,
        "nivel": "Principiante",
        "descripcion": "Rollitos rápidos con sabor a pizza para aprovechar tortillas y queso.",
        "instrucciones": [
            "Extiende una tortilla y coloca salsa de tomate.",
            "Agrega queso y pequeñas porciones de salami.",
            "Enrolla la tortilla con cuidado.",
            "Unta un poco de mantequilla por fuera.",
            "Corta el rollo en pequeñas piezas.",
            "Dora los rollitos en una sartén hasta que el queso se derrita."
        ]
    },

    {
        "nombre": "Burritos crujientes",
        "imagen": "imagenes/burritos_crujientes.png",
        "ingredientes": [
            "salchicha", "crema", "queso",
            "tortilla", "mantequilla", "paprika", "sal"
        ],
        "tiempo": 25,
        "nivel": "Principiante",
        "descripcion": "Burritos sencillos y crujientes rellenos de salchicha y queso.",
        "instrucciones": [
            "Corta la salchicha en trozos pequeños.",
            "Mezcla la salchicha con crema, queso, paprika y sal.",
            "Coloca la mezcla sobre las tortillas.",
            "Enrolla las tortillas formando burritos.",
            "Unta un poco de mantequilla en una sartén.",
            "Dora los burritos por todos sus lados."
        ]
    },

    {
        "nombre": "Bocaditos de papa",
        "imagen": "imagenes/bocaditos_papa.png",
        "ingredientes": [
            "papa", "maicena", "paprika", "sal", "aceite"
        ],
        "tiempo": 30,
        "nivel": "Principiante",
        "descripcion": "Bocaditos crujientes de papa fáciles de preparar.",
        "instrucciones": [
            "Cuece las papas hasta que estén suaves.",
            "Machaca las papas hasta formar un puré.",
            "Agrega maicena, paprika y sal.",
            "Forma pequeños bocaditos con la mezcla.",
            "Calienta aceite en una sartén.",
            "Cocina los bocaditos hasta que estén dorados y crujientes."
        ]
    },

    {
        "nombre": "Rollitos",
        "imagen": "imagenes/rollitos.png",
        "ingredientes": [
            "pan", "queso", "salchicha", "huevo",
            "pan molido", "sal", "aceite"
        ],
        "tiempo": 20,
        "nivel": "Principiante",
        "descripcion": "Rollitos crujientes de pan rellenos de queso y salchicha.",
        "instrucciones": [
            "Aplana las rebanadas de pan.",
            "Coloca queso y salchicha sobre cada rebanada.",
            "Enrolla el pan firmemente.",
            "Bate el huevo con una pizca de sal.",
            "Pasa los rollitos por el huevo y después por pan molido.",
            "Dóralos en aceite hasta que estén crujientes."
        ]
    },

    {
        "nombre": "Caldo de hongos",
        "imagen": "imagenes/caldo_hongos.png",
        "ingredientes": [
            "hongos", "cebolla", "ajo", "epazote",
            "pollo", "chipotle", "queso fresco",
            "caldo de pollo", "aceite"
        ],
        "tiempo": 40,
        "nivel": "Principiante",
        "descripcion": "Caldo reconfortante de hongos con pollo y un toque de chipotle.",
        "instrucciones": [
            "Limpia y corta los hongos.",
            "Sofríe la cebolla y el ajo en un poco de aceite.",
            "Agrega los hongos y cocina durante unos minutos.",
            "Incorpora el pollo y el caldo.",
            "Agrega epazote y chipotle al gusto.",
            "Cocina hasta que los ingredientes estén completamente cocidos.",
            "Sirve con queso fresco."
        ]
    },

    {
        "nombre": "Chayotes a la canela",
        "imagen": "imagenes/chayotes_canela.png",
        "ingredientes": [
            "chayote", "pan molido", "azúcar",
            "canela", "mantequilla"
        ],
        "tiempo": 45,
        "nivel": "Principiante",
        "descripcion": "Una preparación diferente para aprovechar chayotes con un toque dulce.",
        "instrucciones": [
            "Cuece los chayotes hasta que estén suaves.",
            "Córtalos en trozos.",
            "Derrite mantequilla en una sartén.",
            "Agrega los chayotes y espolvorea azúcar y canela.",
            "Cubre con pan molido.",
            "Cocina hasta que estén ligeramente dorados."
        ]
    },

    {
        "nombre": "Rollo de res al horno",
        "imagen": "imagenes/rollo_res_horno.png",
        "ingredientes": [
            "res", "paté", "cebolla", "ajo", "tomillo",
            "mejorana", "mostaza", "harina", "zanahoria",
            "caldo de res", "sal", "pimienta"
        ],
        "tiempo": 90,
        "nivel": "Principiante",
        "descripcion": "Rollo de res relleno y horneado con verduras y hierbas.",
        "instrucciones": [
            "Extiende la carne de res y sazona con sal y pimienta.",
            "Unta una capa de paté y mostaza.",
            "Agrega cebolla, ajo, tomillo, mejorana y zanahoria.",
            "Enrolla la carne y asegúrala para mantener la forma.",
            "Coloca el rollo en una bandeja para horno.",
            "Agrega caldo de res y un poco de harina para formar la salsa.",
            "Hornea hasta que la carne esté completamente cocida.",
            "Deja reposar antes de cortar y servir."
        ]
    },

    {
        "nombre": "Corona de arroz",
        "imagen": "imagenes/corona_arroz.png",
        "ingredientes": [
            "arroz", "crema", "cebolla", "ajo",
            "hongos", "chipotle", "epazote",
            "mantequilla", "caldo de pollo", "pimienta"
        ],
        "tiempo": 40,
        "nivel": "Principiante",
        "descripcion": "Arroz cremoso con hongos y chipotle presentado en forma de corona.",
        "instrucciones": [
            "Sofríe cebolla y ajo con mantequilla.",
            "Agrega los hongos y cocina durante unos minutos.",
            "Incorpora el arroz y mezcla.",
            "Agrega caldo de pollo, crema y chipotle.",
            "Añade epazote y pimienta.",
            "Cocina hasta que el arroz absorba el líquido.",
            "Coloca el arroz en un molde de corona y desmolda para servir."
        ]
    },

    {
        "nombre": "Queso asado",
        "imagen": "imagenes/queso_asado.png",
        "ingredientes": [
            "pan integral", "queso manchego", "nuez", "mostaza"
        ],
        "tiempo": 15,
        "nivel": "Principiante",
        "descripcion": "Pan integral tostado con queso manchego, nuez y mostaza.",
        "instrucciones": [
            "Unta mostaza sobre el pan integral.",
            "Coloca queso manchego encima.",
            "Agrega nueces picadas.",
            "Coloca el pan en una sartén o tostador.",
            "Calienta hasta que el queso se derrita y el pan esté dorado.",
            "Sirve caliente."
        ]
    },

    {
        "nombre": "Soufflé de queso",
        "imagen": "imagenes/souffle_queso.png",
        "ingredientes": [
            "huevo", "queso Chihuahua", "queso manchego",
            "leche", "harina", "mantequilla",
            "cebolla", "caldo de pollo", "pimienta"
        ],
        "tiempo": 45,
        "nivel": "Principiante",
        "descripcion": "Preparación esponjosa de huevo y quesos.",
        "instrucciones": [
            "Derrite mantequilla y sofríe la cebolla.",
            "Agrega harina y mezcla para formar una base.",
            "Incorpora poco a poco la leche y el caldo.",
            "Agrega los quesos y pimienta.",
            "Separa las claras de las yemas y bate las claras hasta que estén espumosas.",
            "Incorpora las yemas a la mezcla de queso.",
            "Integra cuidadosamente las claras batidas.",
            "Coloca la mezcla en un molde y hornea hasta que esté inflada y dorada."
        ]
    },

    {
        "nombre": "Crema de poblano",
        "imagen": "imagenes/crema_poblano.png",
        "ingredientes": [
            "chile poblano", "cebolla", "ajo",
            "queso manchego", "leche", "caldo de pollo",
            "aceite", "queso panela"
        ],
        "tiempo": 25,
        "nivel": "Principiante",
        "descripcion": "Crema suave de chile poblano con queso.",
        "instrucciones": [
            "Asa y limpia los chiles poblanos.",
            "Sofríe la cebolla y el ajo con un poco de aceite.",
            "Licúa los poblanos con la leche y el caldo.",
            "Vierte la mezcla en una olla.",
            "Agrega queso manchego y cocina hasta que se derrita.",
            "Sirve caliente con cubos de queso panela."
        ]
    },

    {
        "nombre": "Verduras en salsa de pavo",
        "imagen": "imagenes/verduras_salsa_pavo.png",
        "ingredientes": [
            "coliflor", "brócoli", "pavo", "cebolla",
            "crema", "mantequilla", "caldo de pollo",
            "sal", "pimienta"
        ],
        "tiempo": 30,
        "nivel": "Principiante",
        "descripcion": "Verduras con pavo en una salsa cremosa.",
        "instrucciones": [
            "Corta la coliflor y el brócoli en trozos pequeños.",
            "Cuece las verduras hasta que estén suaves.",
            "Sofríe la cebolla con mantequilla.",
            "Agrega el pavo y cocina unos minutos.",
            "Incorpora la crema y el caldo de pollo.",
            "Agrega las verduras.",
            "Sazona con sal y pimienta y cocina unos minutos más."
        ]
    },

    {
        "nombre": "Ensalada mediterránea",
        "imagen": "imagenes/ensalada_mediterranea.png",
        "ingredientes": [
            "lechuga", "berros", "zanahoria", "alfalfa",
            "queso manchego", "aceite de oliva", "vinagre",
            "cebolla", "ajo", "pimienta"
        ],
        "tiempo": 15,
        "nivel": "Principiante",
        "descripcion": "Ensalada fresca para aprovechar diferentes verduras y queso.",
        "instrucciones": [
            "Lava y desinfecta las verduras.",
            "Corta la lechuga, zanahoria y cebolla.",
            "Coloca lechuga, berros y alfalfa en un recipiente.",
            "Agrega la zanahoria, cebolla y queso manchego.",
            "Mezcla aceite de oliva, vinagre, ajo y pimienta.",
            "Agrega el aderezo a la ensalada y mezcla."
        ]
    },

    {
        "nombre": "Frutas al Roquefort",
        "imagen": "imagenes/frutas_roquefort.png",
        "ingredientes": [
            "queso Roquefort", "queso crema",
            "pera", "manzana", "limón"
        ],
        "tiempo": 15,
        "nivel": "Principiante",
        "descripcion": "Combinación fresca de frutas con una crema de quesos.",
        "instrucciones": [
            "Lava y corta la pera y la manzana.",
            "Rocía un poco de limón sobre la fruta.",
            "Mezcla el queso Roquefort con el queso crema.",
            "Integra la mezcla de quesos con la fruta.",
            "Refrigera unos minutos antes de servir."
        ]
    },

    {
        "nombre": "Queso empanizado",
        "imagen": "imagenes/queso_empanizado.png",
        "ingredientes": [
            "pan molido", "queso panela", "huevo",
            "caldo de pollo", "pimienta", "aceite"
        ],
        "tiempo": 20,
        "nivel": "Principiante",
        "descripcion": "Queso panela cubierto con una capa crujiente.",
        "instrucciones": [
            "Corta el queso panela en piezas.",
            "Bate el huevo con caldo de pollo y pimienta.",
            "Pasa cada pieza de queso por el huevo.",
            "Cubre completamente con pan molido.",
            "Calienta aceite en una sartén.",
            "Dora el queso por ambos lados hasta que esté crujiente."
        ]
    },

    {
        "nombre": "Hot dog de carrito",
        "imagen": "imagenes/hot_dog_carrito.png",
        "ingredientes": [
            "pan para hot dog", "salchicha", "tocino",
            "cebolla", "tomate", "chile verde", "limón",
            "mostaza", "mantequilla", "mayonesa", "catsup"
        ],
        "tiempo": 25,
        "nivel": "Principiante",
        "descripcion": "Versión casera de un hot dog estilo carrito.",
        "instrucciones": [
            "Envuelve las salchichas con tocino.",
            "Cocina las salchichas hasta que el tocino esté dorado.",
            "Pica la cebolla, tomate y chile verde.",
            "Calienta los panes con un poco de mantequilla.",
            "Coloca la salchicha dentro del pan.",
            "Agrega cebolla, tomate y chile.",
            "Añade mayonesa, mostaza, catsup y unas gotas de limón."
        ]
    },

    {
        "nombre": "Tostadas de manzana",
        "imagen": "imagenes/tostadas_manzana.png",
        "ingredientes": [
            "pan tostado", "manzana", "mantequilla",
            "canela", "azúcar", "queso cottage"
        ],
        "tiempo": 15,
        "nivel": "Principiante",
        "descripcion": "Tostadas dulces con manzana, canela y queso cottage.",
        "instrucciones": [
            "Corta la manzana en rebanadas delgadas.",
            "Derrite mantequilla en una sartén.",
            "Agrega la manzana, canela y azúcar.",
            "Cocina hasta que la manzana esté suave.",
            "Coloca queso cottage sobre el pan tostado.",
            "Agrega las manzanas encima y sirve."
        ]
    },

    {
        "nombre": "Multigrano con tres quesos",
        "imagen": "imagenes/multigrano_tres_quesos.png",
        "ingredientes": [
            "pan multigrano", "queso manchego", "queso gruyere",
            "mozzarella", "alfalfa", "aguacate",
            "mayonesa", "mostaza", "lechuga", "aceite de oliva"
        ],
        "tiempo": 15,
        "nivel": "Principiante",
        "descripcion": "Sandwich multigrano con tres quesos, aguacate y verduras.",
        "instrucciones": [
            "Unta mayonesa y mostaza sobre el pan.",
            "Coloca el queso manchego, gruyere y mozzarella.",
            "Agrega lechuga y alfalfa.",
            "Corta el aguacate y colócalo encima.",
            "Agrega unas gotas de aceite de oliva.",
            "Cierra el sandwich y sirve."
        ]
    },

    {
        "nombre": "Bocadillos integrales",
        "imagen": "imagenes/bocadillos_integrales.png",
        "ingredientes": [
            "pan integral", "pepino", "atún",
            "mayonesa", "mostaza", "nuez", "perejil",
            "mantequilla", "sal", "pimienta"
        ],
        "tiempo": 25,
        "nivel": "Principiante",
        "descripcion": "Bocadillos integrales de atún con pepino y nuez.",
        "instrucciones": [
            "Escurre el atún.",
            "Mézclalo con mayonesa, mostaza y perejil.",
            "Agrega nuez picada y sazona con sal y pimienta.",
            "Corta el pepino en rebanadas delgadas.",
            "Unta mantequilla sobre el pan integral.",
            "Agrega la mezcla de atún y el pepino.",
            "Corta en porciones pequeñas y sirve."
        ]
    },


    # ============================================================
    # NIVEL 2
    # ============================================================

    {
        "nombre": "Rollitos crujientes de ramen y queso",
        "imagen": "imagenes/rollitos_ramen_queso.png",
        "ingredientes": [
            "ramen", "papel de arroz", "queso",
            "harina", "huevo", "pan molido", "aceite"
        ],
        "tiempo": 35,
        "nivel": "Intermedio",
        "descripcion": "Rollitos crujientes rellenos de ramen y queso.",
        "instrucciones": [
            "Prepara el ramen y deja que se enfríe un poco.",
            "Coloca ramen y queso sobre una hoja de papel de arroz.",
            "Enrolla cuidadosamente formando un rollito.",
            "Pasa el rollito por harina.",
            "Después pásalo por huevo batido y pan molido.",
            "Calienta aceite y dora los rollitos hasta que estén crujientes.",
            "Escurre el exceso de aceite antes de servir."
        ]
    },

    {
        "nombre": "Spaghetti tres quesos",
        "imagen": "imagenes/spaghetti_tres_quesos.png",
        "ingredientes": [
            "spaghetti", "queso manchego", "queso crema",
            "queso Roquefort", "crema", "cebolla", "ajo",
            "mantequilla", "leche", "caldo de pollo", "pimienta"
        ],
        "tiempo": 35,
        "nivel": "Intermedio",
        "descripcion": "Spaghetti con una salsa cremosa de tres quesos.",
        "instrucciones": [
            "Cuece el spaghetti hasta que esté al dente.",
            "Derrite mantequilla y sofríe la cebolla y el ajo.",
            "Agrega crema, leche y caldo de pollo.",
            "Incorpora queso manchego, queso crema y Roquefort.",
            "Mezcla hasta obtener una salsa cremosa.",
            "Agrega pimienta al gusto.",
            "Incorpora el spaghetti y mezcla bien.",
            "Sirve caliente."
        ]
    },

    {
        "nombre": "Bimburguesas",
        "imagen": "imagenes/bimburguesas.png",
        "ingredientes": [
            "pan para hamburguesa", "carne molida", "avena",
            "huevo", "cebolla", "queso amarillo",
            "lechuga", "tomate", "jamón",
            "mayonesa", "catsup", "mostaza"
        ],
        "tiempo": 35,
        "nivel": "Intermedio",
        "descripcion": "Hamburguesas caseras con una mezcla de carne y avena.",
        "instrucciones": [
            "Mezcla la carne molida con avena, huevo y cebolla picada.",
            "Forma las hamburguesas.",
            "Cocina las hamburguesas en una sartén hasta que estén completamente cocidas.",
            "Agrega queso amarillo durante los últimos minutos.",
            "Calienta los panes.",
            "Coloca lechuga, tomate y jamón sobre el pan.",
            "Agrega la hamburguesa y los condimentos.",
            "Cierra la hamburguesa y sirve."
        ]
    },

    {
        "nombre": "Canastitas de sierra ahumada",
        "imagen": "imagenes/canastitas_sierra.png",
        "ingredientes": [
            "pan integral", "sierra ahumada", "zanahoria",
            "apio", "mayonesa", "cebolla",
            "perejil", "caldo de pollo", "pimienta"
        ],
        "tiempo": 30,
        "nivel": "Intermedio",
        "descripcion": "Canastitas de pan integral rellenas de sierra ahumada y verduras.",
        "instrucciones": [
            "Aplana las rebanadas de pan integral.",
            "Coloca el pan en moldes pequeños para formar canastitas.",
            "Hornea hasta que estén ligeramente doradas.",
            "Desmenuza la sierra ahumada.",
            "Mezcla con zanahoria, apio, cebolla, mayonesa y perejil.",
            "Agrega un poco de caldo de pollo y pimienta.",
            "Rellena las canastitas con la mezcla."
        ]
    },

    {
        "nombre": "Chuletas al chipotle",
        "imagen": "imagenes/chuletas_chipotle.png",
        "ingredientes": [
            "chuleta de cerdo", "chipotle", "cebollín",
            "ajo", "crema", "caldo de pollo",
            "aceite", "sal", "pimienta"
        ],
        "tiempo": 35,
        "nivel": "Intermedio",
        "descripcion": "Chuletas de cerdo en una salsa cremosa de chipotle.",
        "instrucciones": [
            "Sazona las chuletas con sal y pimienta.",
            "Calienta aceite en una sartén.",
            "Cocina las chuletas por ambos lados hasta que estén completamente cocidas.",
            "Sofríe el ajo y el cebollín.",
            "Licúa crema, chipotle y caldo de pollo.",
            "Agrega la salsa a la sartén.",
            "Cocina unos minutos hasta que la salsa espese.",
            "Regresa las chuletas a la salsa y sirve."
        ]
    },

    {
        "nombre": "Arroz con un poco de todo",
        "imagen": "imagenes/arroz_todo.png",
        "ingredientes": [
            "arroz", "fideo", "mantequilla", "cebolla",
            "ajo", "pollo", "caldo de pollo",
            "puré de tomate", "pimiento rojo", "sal"
        ],
        "tiempo": 40,
        "nivel": "Intermedio",
        "descripcion": "Arroz completo para aprovechar diferentes ingredientes disponibles.",
        "instrucciones": [
            "Derrite mantequilla y sofríe el fideo.",
            "Agrega el arroz y cocina hasta que esté ligeramente dorado.",
            "Añade cebolla y ajo.",
            "Incorpora el pollo y el pimiento rojo.",
            "Agrega caldo de pollo y puré de tomate.",
            "Sazona con sal.",
            "Cocina tapado hasta que el arroz esté suave y haya absorbido el líquido."
        ]
    },

    {
        "nombre": "Cuadritos de ternera",
        "imagen": "imagenes/cuadritos_ternera.png",
        "ingredientes": [
            "ternera", "pan blanco", "hongos",
            "mantequilla", "harina", "crema",
            "zanahoria", "caldo de pollo", "sal", "pimienta"
        ],
        "tiempo": 40,
        "nivel": "Intermedio",
        "descripcion": "Cuadritos de ternera acompañados de una salsa cremosa con hongos.",
        "instrucciones": [
            "Corta la ternera en cubos pequeños.",
            "Sazona con sal y pimienta.",
            "Derrite mantequilla y cocina la ternera.",
            "Agrega los hongos y la zanahoria.",
            "Añade harina y mezcla.",
            "Incorpora poco a poco el caldo de pollo y la crema.",
            "Cocina hasta que la salsa espese y la carne esté completamente cocida.",
            "Sirve con trozos de pan."
        ]
    },

    {
        "nombre": "Tiritas de pescado capeadas",
        "imagen": "imagenes/tiritas_pescado.png",
        "ingredientes": [
            "pescado", "harina", "huevo", "cerveza",
            "caldo de pollo", "aceite", "tomate",
            "cebolla", "cilantro", "limón", "salsa picante"
        ],
        "tiempo": 35,
        "nivel": "Intermedio",
        "descripcion": "Tiras de pescado con una cubierta crujiente y acompañamiento fresco.",
        "instrucciones": [
            "Corta el pescado en tiras.",
            "Prepara una mezcla con harina, huevo y líquido suficiente para formar una capa ligera.",
            "Pasa las tiras de pescado por la mezcla.",
            "Calienta aceite en una sartén.",
            "Cocina las tiras hasta que estén doradas y el pescado esté completamente cocido.",
            "Para acompañar, mezcla tomate, cebolla, cilantro y limón.",
            "Sirve las tiras con la mezcla de verduras y salsa picante al gusto."
        ]
    },

    {
        "nombre": "Ruedas de ternera al tinto",
        "imagen": "imagenes/ruedas_ternera.png",
        "ingredientes": [
            "ternera", "ajo", "cebollín", "albahaca",
            "queso parmesano", "pan molido", "mantequilla",
            "vino tinto", "crema", "pan"
        ],
        "tiempo": 40,
        "nivel": "Intermedio",
        "descripcion": "Ruedas de ternera rellenas y acompañadas de una salsa cremosa.",
        "instrucciones": [
            "Extiende la ternera y sazona.",
            "Mezcla ajo, cebollín, albahaca, queso parmesano y pan molido.",
            "Coloca la mezcla sobre la ternera.",
            "Enrolla y corta en piezas.",
            "Derrite mantequilla y cocina las ruedas hasta que estén doradas.",
            "Prepara la salsa con crema y el líquido de cocción.",
            "Cocina hasta que la carne esté completamente cocida.",
            "Sirve con pan."
        ]
    },

    {
        "nombre": "Balsas de pavo al yogurt",
        "imagen": "imagenes/balsas_pavo_yogurt.png",
        "ingredientes": [
            "pollo", "pan integral", "yogurt natural",
            "queso crema", "cebolla", "mostaza",
            "aceitunas", "mantequilla", "crema",
            "harina", "caldo de pollo", "pimienta"
        ],
        "tiempo": 35,
        "nivel": "Intermedio",
        "descripcion": "Bocadillos de pollo con una mezcla cremosa de yogurt y queso.",
        "instrucciones": [
            "Cocina el pollo y córtalo en trozos pequeños.",
            "Mezcla yogurt natural, queso crema y mostaza.",
            "Agrega cebolla y aceitunas picadas.",
            "Incorpora el pollo y mezcla.",
            "Forma pequeñas bases o balsas con pan integral.",
            "Prepara una salsa con mantequilla, harina, crema y caldo de pollo.",
            "Sirve el pollo sobre el pan y acompaña con la salsa."
        ]
    },

    {
        "nombre": "Rollo de pan con queso",
        "imagen": "imagenes/rollo_pan_queso.png",
        "ingredientes": [
            "pan", "huevo", "mostaza", "queso fresco",
            "mantequilla", "pimiento rojo", "queso crema",
            "cebolla", "ajonjolí"
        ],
        "tiempo": 40,
        "nivel": "Intermedio",
        "descripcion": "Rollos de pan rellenos de queso y verduras.",
        "instrucciones": [
            "Aplana las rebanadas de pan.",
            "Unta mostaza y queso crema.",
            "Agrega queso fresco, cebolla y pimiento rojo picados.",
            "Enrolla cada rebanada.",
            "Pasa los rollos por huevo batido.",
            "Cubre con ajonjolí.",
            "Dora en una sartén con mantequilla hasta que estén crujientes."
        ]
    },

    {
        "nombre": "Triángulos Bimbo",
        "imagen": "imagenes/triangulos_bimbo.png",
        "ingredientes": [
            "pan integral", "mayonesa", "apio",
            "pimiento verde", "carne enlatada",
            "queso gruyere", "col", "mantequilla", "ajonjolí"
        ],
        "tiempo": 30,
        "nivel": "Intermedio",
        "descripcion": "Triángulos de pan integral rellenos y dorados.",
        "instrucciones": [
            "Pica el apio, pimiento verde y col.",
            "Mezcla las verduras con la carne enlatada y mayonesa.",
            "Agrega queso gruyere.",
            "Coloca la mezcla sobre el pan integral.",
            "Cubre con otra rebanada y corta en triángulos.",
            "Unta mantequilla y agrega ajonjolí.",
            "Dora los triángulos hasta que estén crujientes."
        ]
    },

    {
        "nombre": "Salpicadas de ajonjolí",
        "imagen": "imagenes/salpicadas_ajonjoli.png",
        "ingredientes": [
            "pan integral", "pollo", "apio",
            "cebollín", "cebolla", "ajo", "menta",
            "mayonesa", "catsup", "salsa de soya",
            "mantequilla", "ajonjolí"
        ],
        "tiempo": 35,
        "nivel": "Intermedio",
        "descripcion": "Bocadillos de pollo con verduras, salsas y ajonjolí.",
        "instrucciones": [
            "Cocina el pollo y córtalo en trozos pequeños.",
            "Pica apio, cebollín, cebolla, ajo y menta.",
            "Mezcla el pollo con las verduras.",
            "Agrega mayonesa, catsup y salsa de soya.",
            "Coloca la mezcla sobre pan integral.",
            "Unta mantequilla por fuera.",
            "Dora los bocadillos y termina con ajonjolí."
        ]
    },

    {
        "nombre": "Timbales Bimbo",
        "imagen": "imagenes/timbales_bimbo.png",
        "ingredientes": [
            "pan multigrano", "mantequilla", "mostaza",
            "frijoles refritos", "cebollín", "queso",
            "jamón de pavo", "lechuga", "yogurt",
            "vinagre", "aceite de oliva", "chile"
        ],
        "tiempo": 40,
        "nivel": "Intermedio",
        "descripcion": "Timbales de pan multigrano con frijoles, queso y jamón.",
        "instrucciones": [
            "Aplana las rebanadas de pan multigrano.",
            "Unta mantequilla y mostaza.",
            "Agrega una capa de frijoles refritos.",
            "Añade queso, jamón de pavo y cebollín.",
            "Forma los timbales utilizando un molde.",
            "Prepara un aderezo con yogurt, vinagre y aceite de oliva.",
            "Sirve los timbales con lechuga, chile y el aderezo."
        ]
    },

    {
        "nombre": "Pechugas al pimiento",
        "imagen": "imagenes/pechugas_pimiento.png",
        "ingredientes": [
            "pollo", "pimiento verde", "ajo",
            "cebolla", "caldo de pollo", "crema",
            "queso crema", "sal"
        ],
        "tiempo": 40,
        "nivel": "Intermedio",
        "descripcion": "Pechugas de pollo en una salsa cremosa de pimiento verde.",
        "instrucciones": [
            "Sazona el pollo con sal.",
            "Corta el pimiento y la cebolla.",
            "Sofríe el ajo, la cebolla y el pimiento.",
            "Agrega el pollo y cocina por ambos lados.",
            "Licúa parte del pimiento con caldo de pollo, crema y queso crema.",
            "Vierte la salsa sobre el pollo.",
            "Cocina hasta que el pollo esté completamente cocido y la salsa espese.",
            "Sirve caliente."
        ]
    }

]

    # ============================================================
    # POSTRES
    # ============================================================

recetas = [
    {
        "nombre": "Copas Bimbo",
        "imagen": "imagenes/copas_bimbo.png",
        "ingredientes": [
            "1 panqué con nueces Bimbo",
            "300 g de zarzamoras bien lavadas y desinfectadas",
            "1/2 taza de crema espesa",
            "1/2 taza de azúcar",
            "1 cucharadita de vainilla",
            "1 queso crema chico",
            "4 cucharadas de licor de naranja"
        ],
        "tiempo": 15,
        "nivel": "Principiante",
        "descripcion": "Un postre cremoso e individual montado en capas con panqué, crema dulce y zarzamoras.",
        "instrucciones": [
            "Lícue el queso crema, la crema, el azúcar, la vainilla y el licor de naranja.",
            "En cada copa coloque una rebanada de panqué, después ponga las zarzamoras.",
            "Báñelas con la salsa y repita el proceso hasta terminar.",
            "Adorne con zarzamoras y hojitas.",
            "Refrigere antes de servir."
        ]
    },
    {
        "nombre": "Mousse de Chocolate a la Francesa",
        "imagen": "imagenes/mousse_chocolate_francesa.png",
        "ingredientes": [
            "2 tablillas de chocolate Ibarra",
            "1 copa de Grand Marnier o licor de naranja al gusto",
            "1 queso crema grande"
        ],
        "tiempo": 15,
        "nivel": "Principiante",
        "descripcion": "Un suave y elegante mousse de chocolate con toque cítrico de licor.",
        "instrucciones": [
            "Se derrite el chocolate a baño maría y se licúa bien con el queso y el licor.",
            "Se pone en copas individuales.",
            "Adorne con 2 hojitas de hierbabuena o alguna flor.",
            "Refrigere un rato antes de servir."
        ]
    },
    {
        "nombre": "Galletas a la Naranja",
        "imagen": "imagenes/galletas_naranja.png",
        "ingredientes": [
            "1/4 kg de manteca",
            "1/2 kg de harina",
            "1 1/2 taza de azúcar",
            "2 cucharaditas de royal",
            "2 huevos",
            "Ralladura de una naranja",
            "Jugo de una naranja",
            "3/4 de taza de azúcar (para espolvorear)",
            "Canela en polvo (para espolvorear)"
        ],
        "tiempo": 30,
        "nivel": "Explorador",
        "descripcion": "Tradicionales galletas horneadas con un toque cítrico y aromático a naranja y canela.",
        "instrucciones": [
            "Bata y acreme la manteca con el azúcar.",
            "Agregue los huevos y la ralladura. Añada la harina y mezcle con la yema de los dedos.",
            "Poco a poco vaya poniéndole el jugo hasta obtener una masa suave.",
            "Extienda con un rodillo y corte las galletas; colóquelas en una charola engrasada.",
            "Hornee de 9 a 10 minutos o hasta que empiecen a dorar.",
            "Al sacarlas, espolvoree con canela y azúcar mezcladas."
        ]
    },
    {
        "nombre": "Panqué Nevado",
        "imagen": "imagenes/panque_nevado.png",
        "ingredientes": [
            "1 panqué de nuez Bimbo",
            "2 tazas de azúcar",
            "1 cucharadita de vainilla",
            "1/2 taza de agua",
            "4 cucharadas de brandy",
            "1/2 taza de almendras peladas, finamente picadas",
            "3/4 de taza de nuez finamente picada",
            "1/2 taza de nata o queso cottage",
            "1 clara de huevo",
            "Azúcar para espolvorear",
            "Mantequilla (la necesaria)",
            "Canela en polvo (al gusto)"
        ],
        "tiempo": 60,
        "nivel": "Explorador",
        "descripcion": "Panqué bañado en almíbar y frutos secos, cubierto con un merengue horneado.",
        "instrucciones": [
            "Ponga a calentar 3/4 de taza de azúcar con media taza de agua y mueva continuamente. Agregue la vainilla y deje hervir hasta que esté a punto de miel.",
            "Añada la nata o el queso a la preparación anterior.",
            "En un molde chico previamente engrasado, coloque una cama de panqué y báñelo con la mitad de la miel con nata; agregue un poco de brandy y espolvoree la almendra y la nuez.",
            "Bata la clara a punto de turrón con un poco de azúcar y cubra el panqué formando picos.",
            "Espolvoree un poco de azúcar y canela.",
            "Hornee a 250°C durante 40 minutos aproximadamente o hasta que dore el merengue al gusto.",
            "Apague el horno y sirva inmediatamente."
        ]
    },
    {
        "nombre": "Chocolatines",
        "imagen": "imagenes/chocolatines.png",
        "ingredientes": [
            "1 paquete de chocolate amargo",
            "1/2 taza de mantequilla",
            "4 huevos",
            "1/4 de cucharadita de sal",
            "2 tazas de azúcar",
            "1 cucharadita de vainilla",
            "1 taza de harina cernida",
            "1 taza de nuez picada",
            "1 queso crema chico",
            "3 tablillas de chocolate semidulce (cubierta)",
            "3 cucharadas de mantequilla (cubierta)",
            "Nueces enteras (cubierta)"
        ],
        "tiempo": 45,
        "nivel": "Explorador",
        "descripcion": "Cuadritos esponjosos de chocolate y nuez cubiertos con un delicioso glaseado de chocolate.",
        "instrucciones": [
            "Derrita el chocolate con la mantequilla a baño maría.",
            "Mientras tanto, bata los huevos con el azúcar y la sal hasta que se mezclen bien y aclaren.",
            "Cuando el chocolate esté tibio, incorpórelo a los huevos.",
            "Agregue el queso en trocitos, la harina y la nuez, y bata de nuevo.",
            "Coloque la mezcla en una charola engrasada y enharinada, y extiéndala bien.",
            "Hornee a 200°C durante unos 25 minutos aproximadamente.",
            "Para la cubierta: Derrita la mantequilla y el chocolate semidulce a baño maría hasta que espese.",
            "Ya frío el pan, córtelo en cuadros, úntelo con la cubierta de chocolate y adorne con nueces."
        ]
    },
    {
        "nombre": "Pastel de Zanahoria",
        "imagen": "imagenes/pastel_zanahoria.png",
        "ingredientes": [
            "1 1/2 taza de azúcar granulada",
            "1/2 taza de azúcar moscabada",
            "1 1/2 taza de aceite vegetal",
            "2 tazas de harina cernida",
            "1 1/2 cucharadita de polvo para hornear",
            "2 cucharadas de canela",
            "1 cucharadita de bicarbonato de sodio",
            "1 cucharadita de sal",
            "5 huevos",
            "2 1/2 tazas de zanahoria rallada",
            "1 taza de nueces picadas",
            "1 cucharada de harina",
            "1 cucharada de ralladura de limón",
            "1 queso crema grande (betún)",
            "1 barra de mantequilla (betún)",
            "2 tazas de azúcar glass (betún)"
        ],
        "tiempo": 80,
        "nivel": "Explorador",
        "descripcion": "El clásico pastel de zanahoria especiado cubierto con un cremoso betún de queso.",
        "instrucciones": [
            "Precaliente el horno a 180°C y unte con mantequilla el molde.",
            "Bata las dos clases de azúcar y el aceite en un tazón. En otro tazón cierna la harina, el polvo para hornear, el bicarbonato, la canela y la sal. Incorpórelos a la mezcla.",
            "Agregue uno por uno los huevos sin dejar de batir.",
            "Espolvoree la cucharada de harina sobre las nueces. Integre las zanahorias, las nueces y la ralladura de limón a la pasta.",
            "Vacíe al molde y hornee de 60 a 70 minutos aproximadamente. Déjelo reposar 5 minutos antes de desmoldar.",
            "Para el betún: Bata el queso crema, la mantequilla y el azúcar glass, y cubra el pastel ya frío."
        ]
    },
    {
        "nombre": "Copas de Durazno",
        "imagen": "imagenes/copas_durazno.png",
        "ingredientes": [
            "6 duraznos maduros pelados y sin hueso (o de lata)",
            "3/4 de taza de azúcar",
            "1/2 taza de crema dulce",
            "1 sobre de grenetina"
        ],
        "tiempo": 20,
        "nivel": "Principiante",
        "descripcion": "Un postre refrescante de crema de durazno servido en copas individuales.",
        "instrucciones": [
            "Pele cinco duraznos y póngalos a hervir con azúcar hasta que estén suaves (si usa de lata, omita este hervor).",
            "Licúelos con un poco de azúcar si es necesario.",
            "Agregue la crema y el sobre de grenetina previamente disuelto en agua.",
            "Vierta la mezcla en copas.",
            "Adorne con el durazno restante cortado en gajitos y una hojita dulce.",
            "Refrigere hasta que cuaje."
        ]
    },
    {
        "nombre": "Pie de Peras",
        "imagen": "imagenes/pie_peras.png",
        "ingredientes": [
            "2 tazas de vino tinto",
            "3/4 de taza de azúcar",
            "1 raja de canela",
            "5 peras",
            "1 taza de nata o queso cottage",
            "3 cucharadas de azúcar morena",
            "1 cucharadita de vainilla",
            "1/2 taza de mermelada de durazno",
            "1 1/4 de taza de harina (para la pasta)",
            "1 pizca de sal (para la pasta)",
            "5 cucharadas de azúcar (para la pasta)",
            "1 1/2 barras de mantequilla (para la pasta)",
            "1 huevo (para la pasta)",
            "2 cucharadas de agua (para la pasta)"
        ],
        "tiempo": 60,
        "nivel": "Experto",
        "descripcion": "Tarta crujiente rellena de queso dulce y peras pochadas al vino tinto.",
        "instrucciones": [
            "Para el relleno: Ponga a hervir el vino, el azúcar, la canela y 1/2 taza de agua. Pele las peras, partalas por la mitad y retira las semillas. Hiervalas 15 minutos en el vino. Retire del fuego y refrigere.",
            "Para la pasta: Incorpore la harina, la sal y el azúcar. Mezcle la mantequilla en cuadritos usando un tenedor hasta formar una mezcla arenosa. Agregue el huevo y el agua hasta integrar.",
            "Extienda la masa con rodillo y colóquela sobre un molde para pie previamente engrasado.",
            "Hornee la base a 200°C hasta que se cueza y dore.",
            "Mezcle la nata o queso con el azúcar morena y la vainilla.",
            "Sobre la base horneada ponga la crema de queso, acomode las peras y barnice con la mermelada de durazno derretida."
        ]
    },
    {
        "nombre": "Madalenas con Fresas",
        "imagen": "imagenes/madalenas_fresas.png",
        "ingredientes": [
            "36 fresas limpias y desinfectadas",
            "12 madalenas",
            "1/2 barra de mantequilla",
            "1/2 taza de ron",
            "1/2 taza de azúcar"
        ],
        "tiempo": 15,
        "nivel": "Principiante",
        "descripcion": "Madalenas y fresas bañadas en un jarabe caliente de mantequilla y ron.",
        "instrucciones": [
            "Se derrite la mantequilla a fuego lento.",
            "Se retira del fuego y se agrega el azúcar hasta que se disuelva.",
            "Vierta el ron.",
            "Se vuelve a poner a fuego hasta que caliente (sin llegar a ebullición).",
            "En un platito se acomodan 2 madalenas y 8 fresas.",
            "Báñelas con la salsa caliente y sirva inmediatamente."
        ]
    },
    {
        "nombre": "Hot Cake Tropical",
        "imagen": "imagenes/hot_cake_tropical.png",
        "ingredientes": [
            "1 paquete de harina para Hot Cakes",
            "2 cucharadas de mantequilla",
            "1 huevo",
            "Leche (la necesaria)",
            "Fresas, kiwi o fruta de la estación",
            "100 g de queso cottage",
            "Miel de abeja (al gusto)",
            "Mantequilla extra para la sartén"
        ],
        "tiempo": 20,
        "nivel": "Principiante",
        "descripcion": "Hot cakes esponjosos acompañados de frutas frescas, queso cottage y miel.",
        "instrucciones": [
            "Prepare una pasta mezclando la harina, la mantequilla, el huevo y la leche necesaria hasta integrar perfectamente.",
            "En una sartén con mantequilla, vierta un cucharón de la mezcla y mueva para esparcir y dorar parejo.",
            "Sirva los hot cakes en platos, acomode las frutas escogidas y un poco de queso cottage encima.",
            "Bañe con miel de abeja y adorne con fruta fresca."
        ]
    },
    {
        "nombre": "Pan Francés",
        "imagen": "imagenes/pan_frances.png",
        "ingredientes": [
            "8 rebanadas de Pan Blanco Bimbo",
            "1/4 de taza de nuez picada",
            "3 huevos",
            "2 cucharadas de coulant o licor de naranja",
            "1/2 taza de azúcar morena",
            "2 cucharadas de canela molida",
            "2 cucharadas de leche",
            "Raspadura de media naranja",
            "Aceite (el suficiente para freír)"
        ],
        "tiempo": 15,
        "nivel": "Principiante",
        "descripcion": "Pan francés aromatizado con licor y ralladura de naranja, empanizado en azúcar y canela.",
        "instrucciones": [
            "Bata bien los huevos con un tenedor y agregue la leche, el licor y la raspadura de naranja.",
            "Corte las rebanadas de pan en círculos y sumérjalos en la mezcla de huevo.",
            "En una sartén con aceite caliente, dore el pan por ambos lados y colóquelo sobre papel absorbente.",
            "Espolvoree con la canela y el azúcar mezcladas previamente.",
            "Añada la nuez picada, sirva y adorne con gajos y raspadura de naranja."
        ]
    },
    {
        "nombre": "Yogurt Completo",
        "imagen": "imagenes/yogurt_completo.png",
        "ingredientes": [
            "1 1/2 taza de granola",
            "2 tazas de yogurt natural",
            "1 taza de fresas",
            "1 pera en rebanadas",
            "1 plátano rebanado",
            "Miel de maple (al gusto)"
        ],
        "tiempo": 10,
        "nivel": "Principiante",
        "descripcion": "Parfait fresco montado en capas de yogurt, granola y variedad de frutas de estación.",
        "instrucciones": [
            "Ponga en varias copas una capa de granola, una de yogurt y frutas rebanadas (fresas y peras).",
            "Coloque otra capa idéntica de yogurt con las frutas encima.",
            "Adorne con fruta fresca de temporada y bañe con miel de maple al gusto."
        ]
    },
    {
        "nombre": "Arroz con Leche Tradicional",
        "imagen": "imagenes/arroz_con_leche_tradicional.png",
        "ingredientes": [
            "4 1/2 tazas de agua",
            "1 raja de canela",
            "1 taza de arroz (remojado en agua caliente por 20 min y escurrido)",
            "1 lata de Leche Condensada LA LECHERA®",
            "1 lata de Leche Evaporada CARNATION® CLAVEL®",
            "1/2 taza de pasas",
            "1 cucharada de canela molida"
        ],
        "tiempo": 60,
        "nivel": "Principiante",
        "descripcion": "Clásico postre cremoso de arroz aromatizado con canela y endulzado con leche condensada.",
        "instrucciones": [
            "En una olla, calienta el agua junto con la raja de canela hasta que suelte el hervor.",
            "Agrega el arroz escurrido y cocina a fuego medio durante 15 minutos o hasta que esté tierno.",
            "Vierte la Leche Condensada, la Leche Evaporada y las pasas.",
            "Mezcla bien y cocina a fuego bajo durante 20 a 25 minutos, moviendo ocasionalmente hasta que espese.",
            "Sirva tibio o frío espolvoreado con la canela molida."
        ]
    },
    {
        "nombre": "Arroz con Leche Choco Cereza",
        "imagen": "imagenes/arroz_choco_cereza.png",
        "ingredientes": [
            "4 1/2 tazas de agua",
            "1 raja de canela",
            "1 taza de arroz (remojado en agua caliente por 20 min y escurrido)",
            "1 lata de leche condensada",
            "1 lata de leche evaporada",
            "1 barra de chocolate amargo troceado (150 g)",
            "10 cerezas rojas en almíbar, picadas",
            "Crema batida, cerezas y hojas de menta (para decorar)"
        ],
        "tiempo": 55,
        "nivel": "Principiante",
        "descripcion": "Variación gourmet del arroz con leche tradicional con chocolate amargo fundido y cerezas.",
        "instrucciones": [
            "Calienta el agua con la canela en una olla hasta hervir.",
            "Agrega el arroz escurrido y cocina a fuego alto durante 20 minutos hasta que esté suave.",
            "Incorpora la leche condensada, la leche evaporada y el chocolate amargo troceado.",
            "Mezcla bien y cocina a fuego bajo durante 15 minutos más hasta que espese ligeramente.",
            "Retira del fuego, añade las cerezas picadas y mezcla suavemente.",
            "Deja enfriar y sirva tibio o a temperatura ambiente.",
            "Decora cada porción con crema batida, cerezas en almíbar y hojas de menta."
        ]
    },
    {
        "nombre": "Donas de Avena",
        "imagen": "imagenes/donas_avena.png",
        "ingredientes": [
            "1 1/2 tazas de bebida vegetal de avena sabor vainilla",
            "2 huevos",
            "1 taza de harina para hot cakes",
            "1/2 taza de hojuelas de avena",
            "1 cucharada de canela molida",
            "Aceite en aerosol",
            "1/2 barra de chocolate amargo (75 g)"
        ],
        "tiempo": 30,
        "nivel": "Principiante",
        "descripcion": "Donas horneadas nutritivas a base de avena con cobertura de chocolate amargo.",
        "instrucciones": [
            "Licúa la bebida de avena, los huevos, la harina, las hojuelas de avena y la canela hasta obtener una masa homogénea.",
            "Rocía aceite en aerosol sobre un molde para donas.",
            "Vierte la mezcla en el molde llenando a 3/4 de su capacidad.",
            "Hornee a 180°C durante 20 minutos hasta que estén firmes y cocidas.",
            "Funde el chocolate amargo a baño María.",
            "Desmolda las donas frías y decora la parte superior con el chocolate fundido."
        ]
    },
    {
        "nombre": "Gelatina de Mazapán",
        "imagen": "imagenes/gelatina_mazapan.png",
        "ingredientes": [
            "1 Lata de Leche Condensada LA LECHERA®",
            "1 Lata de Leche Evaporada CARNATION® CLAVEL®",
            "1 Lata de Media Crema NESTLÉ®",
            "3 Tazas de agua",
            "6 Mazapanes (28 g c/u)",
            "3 Sobres de grenetina (7 g c/u)",
            "1 Taza de fresas desinfectadas"
        ],
        "tiempo": 20,
        "nivel": "Principiante",
        "descripcion": "Gelatina cremosa preparada con mazapán tradicional de cacahuate y fresas frescas.",
        "instrucciones": [
            "Hidrata la grenetina en 1/2 taza de agua y disuélvela a baño María.",
            "Licúa la Leche Evaporada, Leche Condensada, Media Crema, las 3 tazas de agua y 5 mazapanes.",
            "Con la licuadora encendida, agrega poco a poco la grenetina disuelta.",
            "Vierte en un molde para gelatina previamente engrasado.",
            "Refrigera hasta que cuaje por completo.",
            "Desmolda y decora con el mazapán restante espolvoreado y las fresas."
        ]
    },
    {
        "nombre": "Pay de Mango",
        "imagen": "imagenes/pay_mango.png",
        "ingredientes": [
            "30 Galletas Marías molidas",
            "1 Barra de mantequilla derretida (90 g)",
            "1 Lata de Leche Condensada LA LECHERA®",
            "3 Mangos",
            "1 1/2 Sobres de grenetina (7 g c/u)"
        ],
        "tiempo": 25,
        "nivel": "Principiante",
        "descripcion": "Pay de mango frío sin horno sobre una crujiente base de galleta María.",
        "instrucciones": [
            "Hidrata la grenetina en 1/4 taza de agua y disuélvela a baño María.",
            "Para la base: Mezcla las galletas molidas con la mantequilla, cúbre la base y bordes de un molde para pay y refrigera 10 minutos.",
            "Para el relleno: Licúa la Leche Condensada con la pulpa de los mangos y, con la licuadora encendida, añade la grenetina.",
            "Vierte el relleno sobre la base de galleta.",
            "Refrigera hasta que cuaje por completo."
        ]
    },
    {
        "nombre": "Pastel Imposible de Moka",
        "imagen": "imagenes/pastel_imposible_moka.png",
        "ingredientes": [
            "1 Taza de cajeta",
            "2 Latas de Leche Evaporada",
            "8 Huevos",
            "1 Taza de aceite vegetal",
            "1 Paquete de harina para pastel de chocolate (232 g)",
            "3 Cucharadas de café soluble",
            "1 Lata de Leche Condensada",
            "3 Cucharadas de esencia de nuez",
            "1 Taza de fresas cortadas en cuartos",
            "1/4 De taza de nuez picada"
        ],
        "tiempo": 110,
        "nivel": "Experto",
        "descripcion": "Espectacular combinación horneada a baño María de pastel de chocolate al café y flan de nuez.",
        "instrucciones": [
            "Cubre el fondo y paredes de un molde engrasado y enharinado con la cajeta.",
            "Licúa 1 lata de Leche Evaporada con 3 huevos, el aceite, la harina de chocolate y el café soluble.",
            "Para el flan: Licúa la Leche Condensada, 1 lata de Leche Evaporada, 5 huevos y la esencia de nuez.",
            "Vacía en el molde primero la mezcla de pastel y luego, con ayuda de una cuchara, vierte poco a poco la del flan.",
            "Cubre con papel aluminio y hornea a 180°C a baño María durante 1 hora y media.",
            "Deja enfriar, desmolda, refrigera 1 hora y decora con las fresas y la nuez."
        ]
    },
    {
        "nombre": "Brownie Cookies and Cream",
        "imagen": "imagenes/brownie_cookies_cream.png",
        "ingredientes": [
            "160 g de chocolate amargo 85%",
            "1 Barra de mantequilla fundida (90 g)",
            "1/2 Taza de Leche Condensada",
            "2 Cucharadas de cocoa",
            "1 Cucharadita de polvo para hornear",
            "3/4 Taza de harina de trigo cernida",
            "3 Huevos",
            "1 Barra de chocolate blanco fundido",
            "1 Paquete de galletas de chocolate con relleno troceadas"
        ],
        "tiempo": 40,
        "nivel": "Explorador",
        "descripcion": "Brownie denso de chocolate decorado con hilos de chocolate blanco y galletas troceadas.",
        "instrucciones": [
            "Mezcla el chocolate amargo con la mantequilla, la Leche Condensada, la cocoa, el polvo para hornear, la harina y los huevos hasta integrar.",
            "Vierte la mezcla en un molde cuadrado cubierto con papel encerado.",
            "Hornee a 180°C durante 25 a 30 minutos y deje enfriar.",
            "Decora la superficie con el chocolate blanco fundido y las galletas troceadas.",
            "Desmolda y corta en cuadros."
        ]
    },
    {
        "nombre": "Brownie Cheesecake con Caramelo",
        "imagen": "imagenes/brownie_cheesecake_caramelo.png",
        "ingredientes": [
            "1 3/4 Taza de azúcar refinada",
            "1 Lata de Media Crema caliente",
            "160 g de chocolate amargo 85%",
            "1 Barra de mantequilla fundida (90 g) + 2 cucharadas extra",
            "2 Cucharadas de cocoa",
            "1 Cucharadita de polvo para hornear",
            "3/4 De taza de harina de trigo cernida",
            "4 Huevos",
            "1 Paquete de queso crema (190 g)",
            "1/4 De taza de Leche Condensada",
            "1 Cucharadita de esencia de vainilla",
            "1 Taza de nuez entera"
        ],
        "tiempo": 55,
        "nivel": "Experto",
        "descripcion": "Combinación marmoleada de brownie de chocolate con cheesecake horneado y salsa de caramelo.",
        "instrucciones": [
            "Para el caramelo: Derrita 3/4 de taza de azúcar a fuego medio. Añada poco a poco la Media Crema caliente y 2 cucharadas de mantequilla hasta integrar.",
            "Para el brownie: Mezcla el chocolate amargo con la barra de mantequilla, 1 taza de azúcar, la cocoa, el polvo para hornear, la harina y 3 huevos. Reserva 1/3 y vierta el resto en un refractario cuadrado con papel encerado.",
            "Para el cheesecake: Bate el queso crema con la Leche Condensada, la vainilla y 1 huevo.",
            "Vierte el cheesecake sobre la base de brownie. Cubre con el resto de la mezcla de brownie reservada y marmolea con un palillo.",
            "Hornee a 180°C durante 30 a 35 minutos. Deja enfriar y desmolda.",
            "Decora con la salsa de caramelo y las nueces enteras."
        ]
    }
]
